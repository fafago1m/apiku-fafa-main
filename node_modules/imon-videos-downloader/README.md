<h3 align="center">

  <p align="center"><img src="https://img.shields.io/badge/WLCM%20TO -IMON VIDEO DOWNLOADER-green?colorA=%23ff0000&colorB=%23017e40&style=flat-square">  

</h3>

[![Typing SVG](https://readme-typing-svg.herokuapp.com?font=Neuton&size=25&color=30FF40&background=000000&center=true&vCenter=true&width=360&height=60&lines=Hello+World%2C+I'm+IMON-404+Here+🤙;𝙸𝚃'𝚜+𝙽𝙾𝚃+𝙰+𝙹𝚄𝚂𝚃+𝙽𝙰𝙼𝙴+𝙱𝚁𝙾+🥱;𝙸𝚃'𝚜+𝙰+𝙱𝚁𝙰𝙽𝙳+🔥;Respect+IMON-404+🥀;Thanks+My+All+Friend+🤙+🥰)](https://git.io/typing-svg)




## Instalation :
```bash
> npm imon-videos-downloader
```

## Example (fb & insta)
```js
const { ndown } = require("imon-videos-downloader")
let URL = await ndown("https://www.facebook.com/100000959749712/posts/pfbid0288xi44nvodK9d7r3wf4LHeM3dtEsVghQXmz5t59axwz7KdLStYyg4qfvTVrAL27Ll/?app=fbl")
console.log(URL)
```
## Output Example
```
{
    "developer": "IMON",
    "devfb": "https://www.facebook.com/Imon.132233",
    "devwp": "https://Wa.me/+8801318582357",
    "status": true,
    "data": [
        {
            "resolution": "720p (HD)",
            "thumbnail": "https://scontent-ber1-1.xx.fbcdn.net/v/t15.5256-10/400486200_1343033686353383_5174403316165836536_n.jpg?_nc_cat=108&ccb=1-7&_nc_sid=dd673f&_nc_ohc=2xRWTn71BMQAX8u2-qw&_nc_ht=scontent-ber1-1.xx&oh=00_AfCzw-5oUCmwmFGGT3_tmPTy0KTuYtmMD4--Ed1gZbXubg&oe=6586FB4E",
            "url": "https://video-ber1-1.xx.fbcdn.net/o1/v/t2/f1/m69/GHvmKhgZQAUkYJ4CAEY4Fvt4bWc6bmdjAAAF.mp4?efg=eyJ2ZW5jb2RlX3RhZyI6Im9lcF9oZCJ9&_nc_ht=video-ber1-1.xx.fbcdn.net&_nc_cat=103&strext=1&vs=44ce93d67fb1cb71&_nc_vs=%2B%%3D%3D&ccb=9-4&oh=00_AfCZS1NarEnG6CaVCjJvCdaVWCRMpf381WpwqUubSjNM_Q&oe=65832C04&_nc_sid=1d576d&_nc_rid=544475056770190&_nc_store_type=1&dl=1",
            "shouldRender": false
        },
        {
            "resolution": "360p (SD)",
            "thumbnail": "https://scontent-ber1-1.xx.fbcdn.net/v/t15.5256-10/400486200_1343033686353383_5174403316165836536_n.jpg?_nc_cat=108&ccb=1-7&_nc_sid=dd673f&_nc_ohc=2xRWTn71BMQAX8u2-qw&_nc_ht=scontent-ber1-1.xx&oh=00_AfCzw-5oUCmwmFGGT3_tmPTy0KTuYtmMD4--Ed1gZbXubg&oe=6586FB4E",
            "url": "https://video-ber1-1.xx.fbcdn.net/v/t42.1790-2/404349742_877328270732084_8635412864009964753_n.mp4?_nc_cat=109&ccb=1-7&_nc_sid=55d0d3&efg=eyJybHIiOjMzMSwicmxhIjo4NTMsInZlbmNvZGVfdGFnIjoic3ZlX3NkIn0%3D&_nc_ohc=CiYYCEuMrVgAX-xk9HO&rl=331&vabr=184&_nc_ht=video-ber1-1.xx&oh=00_AfCl3XhkvJuI5ykViWS98MQ8TUp0HYuRfewCFQ81K9j80Q&oe=6586092D&dl=1",
            "shouldRender": false
        },
        {
            "resolution": "480p",
            "thumbnail": "https://scontent-ber1-1.xx.fbcdn.net/v/t15.5256-10/400486200_1343033686353383_5174403316165836536_n.jpg?_nc_cat=108&ccb=1-7&_nc_sid=dd673f&_nc_ohc=2xRWTn71BMQAX8u2-qw&_nc_ht=scontent-ber1-1.xx&oh=00_AfCzw-5oUCmwmFGGT3_tmPTy0KTuYtmMD4--Ed1gZbXubg&oe=6586FB4E",
            "url": "/render.php?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ2aWRlb191cmwiOiJodHRwczovL3ZpZGVvLWJlcjEtMS54eC5mYmNkbi5uZXQvdi90MzkuMjU0NDctMi80MDU0NjQ2OTlfNzM3MDg1NzExNzk0MjAxXzQyMDg0NTI3NDMxNzcwNTgzNzRfbi5tcDQ_.COR5xzqjXhLneqsiLOt8k6M2WJ4JSu3PyRQR-BIneJU",
            "shouldRender": true
        },
        {
            "resolution": "360p",
            "thumbnail": "https://scontent-ber1-1.xx.fbcdn.net/v/t15.5256-10/400486200_1343033686353383_5174403316165836536_n.jpg?_nc_cat=108&ccb=1-7&_nc_sid=dd673f&_nc_ohc=2xRWTn71BMQAX8u2-qw&_nc_ht=scontent-ber1-1.xx&oh=00_AfCzw-5oUCmwmFGGT3_tmPTy0KTuYtmMD4--Ed1gZbXubg&oe=6586FB4E",
            "url": "/render.php?token=eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9._.mTZkHQCoKkRJ88VxCNydIHUwZ5bhrjr58ExH-_s__lg",
            "shouldRender": true
        }
    ]
}
```

## Example (TikTok)
```js
const { tikdown } = require("imon-videos-downloader")
let URL = await tikdown("https://vt.tiktok.com/ZS2tVBhek/")
console.log(URL)
```

## Output Example (tikTok)
```
"developer": "IMON",
  "devfb": "https://www.facebook.com/Imon.132233",
  "devwp": "https://Wa.me/+8801318582357",
  "status": true,
  "data": {
    "author": {
      "id": "7262042273721533446",
      "unique_id": "farhan_islam_imon",
      "nickname": "farhan islam imon",
      "avatar": "https://p16-sign-sg.tiktokcdn.com/tos-alisg-avt-0068/e1100806cce2af41ccef9d62a6082ff6~tplv-tiktokx-cropcenter:300:300.jpeg?dr=14577&nonce=99460&refresh_token=2d4bb7ecbb11d39126f214ea61aaf977&x-expires=1735318800&x-signature=wfywvURXlIkGzEOwHFRFj1%2B7Bik%3D&idc=maliva&ps=13740610&shcp=d05b14bd&shp=45126217&t=4d5b0474"
    },
    "view": 68,
    "comment": 7,
    "play": 297,
    "share": 1,
    "download": 0,
    "duration": 6,
    "title": "hasi khusi 🥰🥀",
    "thumbnail": "https://p16-sign-sg.tiktokcdn.com/tos-alisg-p-0037/fffba0dda83f40a884593a250d1f3649_1727789604~tplv-tiktokx-360p.jpeg?dr=14555&nonce=55687&refresh_token=ee0222380b238b856554c93bfeb8cc96&x-expires=1735318800&x-signature=XF2FarzyndV8FdMnjyHu1mtYQSU%3D&ftpl=1&idc=maliva&ps=13740610&s=AWEME_DETAIL&shcp=34ff8df6&shp=d05b14bd&t=4d5b0474",
    "video": "https://v16m-default.akamaized.net/fcce881e83e272fcb8b64f9b4e586630/676de1e8/video/tos/alisg/tos-alisg-pve-0037c001/osYQGB8MQAZgGEbRheCpf7Mf3FiAlQPtXe9PgA/?a=0&bti=OUBzOTg7QGo6OjZAL3AjLTAzYCMxNDNg&ch=0&cr=0&dr=0&er=0&lr=all&net=0&cd=0%7C0%7C0%7C0&br=1232&bt=616&cs=0&ds=6&ft=XE5bCqT0majPD122Ww~J3wUOx5EcMeF~O5&mime_type=video_mp4&qs=4&rc=OWQ2NzQ0Mzc1O2lkNDk1aEBpajQzNXU5cnN5dTMzODczNEAxLzQ1YWNeNmExMmI0L2FfYSNmZWQzMmRzZ3FgLS1kMTFzcw%3D%3D&src=1&vvpl=1&l=20241226170817625870476CDDCD325B9C&btag=e000b0000",
    "audio": "https://sf16-ies-music-sg.tiktokcdn.com/obj/tiktok-obj/7404359658384771857.mp3"
  }
}

```

## Instagram Usage 
```js
const {instagram} = require("imon-videos-downloader");

const link = "https://www.instagram.com/p/DAxzjkAyWOm/?igsh=MTZ5eThrMXpteTFpOA==" //past video link

  instagram(link).then(data => {

    console.log(data)

});
```

## Output Example (instagram)
```
{
  "developer": "IMON",
  "devfb": "https://www.facebook.com/Imon.132233",
  "devwp": "https://Wa.me/+8801318582357",
  "status": true,
  "data": {
    "thumb": [
      "video thumbnail"
    ],
    "video": [
      "video video link"
    ],
    "images": []
  }
}
```

## Example (YouTube)
```js
const { ytdown } = require("imon-videos-downloader")
let URL = await ytdown("https://youtu.be/aRSuyrZFu_Q?si=bsfzgeeGmRpsHqnF")
console.log(URL)
```

## Output Example (YouTube)
```

  "developer": "IMON",
  "devfb": "https://www.facebook.com/Imon.132233",
  "devwp": "https://Wa.me/+8801318582357",
  "status": true,
  "data": {
    "title": "video title",
    "thumb": "video thumbnail",
    "video": "normal video link",
    "video_hd": "hd video link",
    "audio": "audio link",
    "quality": "360p/720p",
    "channel": "Not available",
    "desc": "Not available"
  }
}
```
## Example (Twitter)
```js
const { twitterdown } = require("imon-videos-downloader")
let URL = await twitterdown("https://twitter.com/TeamAbhiSha/status/1743351410761019794?t=vms8wxcU0mQuhVxwGCHjFw&s=19")
console.log(URL)
```

## Output Example (Twitter)
```
{
    "developer": "IMON",
    "devfb": "https://www.facebook.com/Imon.132233",
    "devwp": "https://Wa.me/+8801318582357",
    "status": true,
    "data": {
        "HD": "https://video.twimg.com/ext_tw_video/1743351351898181632/pu/vid/avc1/810x720/gKWI2KEyLdRMQBFa.mp4?tag=12",
        "SD": "https://video.twimg.com/ext_tw_video/1743351351898181632/pu/vid/avc1/404x360/dtDrE8AqyxXhoRhO.mp4?tag=12"
    }
}
```

## Usage GDLink 
```js
const {GDLink} = require("imon-videos-downloader");

const url = 'url' // Public Google Drive Url

GDLink(url).then(data => {
  console.log(data)

});
```

## Usage Pintarest 
```js
const {pintarest} = require("imon-videos-downloader");

const url = 'url' // pintarest post url

  pintarest(url).then(data => {
  console.log(data)
    });
```

## Output Example (pintarest)
```

  "developer": "IMON",
  "devfb": "https://www.facebook.com/Imon.132233",
  "devwp": "https://Wa.me/+8801318582357",
  "status": 200,
  "data": {
    "title": "video title",
    "url": "video link",
    "thumbnail": "video thumbnail"
  }
}
```
## Usage CapCut 
```js
const { capcut } = require("imon-videos-downloader");

const url = "link" // capcut link

capcut(url).then(data => {
  console.log(data)
});
```

## Usage Likee 
```js
const { likee} = require("imon-videos-downloader");

 const url = "link" // past url
likee(url).then(data => { 
  console.log(data) 
});
```

## Output Example (Likee)
```
{
  "developer": "IMON",
  "devfb": "https://www.facebook.com/Imon.132233",
  "devwp": "https://Wa.me/+8801318582357",
  "status": 200,
  "data": {
    "title": "video title",
    "thumbnail": "video thumbnail",
    "withWatermark": "video link withWatermark",
    "withoutwatermark": "video link withoutwatermark"
  }
}
```

## Usage Threads
```js
const { threads } = require("imon-videos-downloader");

 const url = "link" // past url
threads(url).then(data => { 
  console.log(data) 
});
```


## Usage All Media Down
```bash
support url: facebook, tiktok, twitter, instagram, youtube, pinterest, gdrive, capcut, likee, threads
note: Let me know if any of the platforms you use are missing
```
```js
const {alldown} = require("imon-videos-downloader");
const url = 'url' // past url

  alldown(url).then(data => {
  console.log(data)
    });
```
## Output Example (alldown)
```
{
    "developer": "IMON",
    "devfb": "https://www.facebook.com/Imon.132233?mibextid=ZbWKwL",
    "devwp": "wa.me/+8801318582357",
    "status": true,
    "media": {
        "title": "video title",
        "low": "normal video link",
        "high": "hd video link"
    }
}
```

[![WhatsApp](https://img.shields.io/badge/WhatsApp-green?style=for-the-badge&logo=whatsapp)](https://wa.me/+8801318582357)
[![Facebook](https://img.shields.io/badge/Facebook-green?style=for-the-badge&logo=facebook)](https://www.facebook.com/Imon.132233?mibextid=ZbWKwL)
[![Messenger](https://img.shields.io/badge/Chat-Messenger-blue?style=for-the-badge&logo=messenger)](https://m.me/100075122837809)
[![Github](https://img.shields.io/badge/Github-MrDarkYTgreen?style=for-the-badge&logo=github)](https://github.com/imon-404)


# developer information 
#### 𝐍𝐚𝐦𝐞        : 𝐌𝐨𝐡𝐚𝐦𝐦𝐚𝐝 𝐈𝐦𝐨𝐧
#### 𝐑𝐞𝐥𝐥𝐠𝐥𝐨𝐧    : (𝐈𝐬𝐥𝐚𝐦)
#### 𝐏𝐞𝐫𝐦𝐚𝐧𝐞𝐭 𝐀𝐝𝐝𝐫𝐞𝐬𝐬 : (𝐃𝐡𝐚𝐤𝐚,𝐁𝐚𝐧𝐠𝐥𝐚𝐝𝐞𝐬𝐡)
#### 𝐂𝐮𝐫𝐫𝐞𝐧𝐭 𝐀𝐝𝐝𝐫𝐞𝐬𝐬 : (𝐁𝐚𝐫𝐢𝐬𝐡𝐚𝐥,𝐁𝐚𝐧𝐠𝐥𝐚𝐝𝐞𝐬𝐡)
#### 𝐆𝐞𝐧𝐝𝐞𝐫     : (𝐌𝐚𝐥𝐞)
#### 𝐀𝐠𝐞            :  (21)
#### 𝐑𝐞𝐥𝐚𝐭𝐢𝐨𝐧𝐬𝐡𝐢𝐩 : (𝐒𝐢𝐧𝐠𝐥𝐞)
#### 𝐓𝐞𝐥𝐞𝐠𝐫𝐚𝐦  : t.me/Farhan_islam12
### [Facebook](https://www.facebook.com/Imon.132233?mibextid=ZbWKwL)